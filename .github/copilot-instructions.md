...
- **ADR-017:** Client component lifecycle (amends ADR-009). Schema `ui.component` overrides are **light-DOM custom elements** (`nagara-*`, no shadow DOM, `display: contents` hosts) extending `NagaraElement` (`public/components/base.mjs`): `static deps` lists the subtree paths they re-render on, subscriptions attach in `connectedCallback` / release in `disconnectedCallback`, and `render(character)` takes its data as the argument and never reads global state. The view binds **only** native `input` / `select` / `textarea` / `output[data-path]` controls to `updateFieldValue()` (which skips `data-editing`); change detection in `state.mjs` is structural (deep-equal leaves, ancestor + root notification, `_`-prefixed transport keys excluded, nothing fires when nothing changed). All client writes go through `api.patchCharacter(id, updates)` (and `api.uploadPortrait(id, file)` for the portrait POST); both resolve with the parsed body on any status — callers branch on `success`. `base.mjs` also exports `componentFactory(ElementClass)` (builds the six-argument registry factory) and `isWritable(fieldSchema, role, mode)`; `rebuild(...nodes)` on the base class is the cleanup → `replaceChildren` → enhance helper. A late response after the sheet was torn down must be dropped (`isConnected` guard). Stable anchors: `§light-dom`, `§deps`, `§render-arg`, `§structural-diff`, `§leaf-binding`, `§patch-helper`.
...
- **UI verification** — after `public/` CSS/HTML/client JS edits, exercise the affected view in-browser via Playwright MCP: [`instructions/ui-browser-verify.instructions.md`](instructions/ui-browser-verify.instructions.md). How to actually drive the current (pre-redesign) UI — create/edit flows, hidden submit control, readonly-until-click fields: [`instructions/ui-navigation-playbook.instructions.md`](instructions/ui-navigation-playbook.instructions.md).
