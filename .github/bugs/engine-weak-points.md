...
- **⚠️ Duplicate id #35:** Two *different* bugs currently share the number 35 — this one and "`RawEffect` wire shape still leaks into the engine" in the **DEFERRED** section. They are unrelated; resolving one does **not** resolve the other, so do not couple their status. Cross-linked so neither is lost. Renumber/de-dup deferred to Pass E.5 (bug-tracker conventions) per user decision 2026-06-07; ids kept stable until then so existing cites resolve.
...
- **⚠️ Duplicate id #35:** Two *different* bugs currently share the number 35 — this one and "`secondary` + `setBase` has no primary-substitution mechanism" in the **HIGH** section. They are unrelated; resolving one does **not** resolve the other, so do not couple their status. Cross-linked so neither is lost. Renumber/de-dup deferred to Pass E.5 (bug-tracker conventions) per user decision 2026-06-07; ids kept stable until then so existing cites resolve.
...
### 36. `enforceConsistency()` may be fully redundant with schema/storage defaulting
...
### 37. `src/rules/registry.mts` re-export shim may have outlived its purpose
...
### 38. Spell-tier `attackAttribute`: authoring spec and audit lint disagree
