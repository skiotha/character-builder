...
- **Domain:** engine
- **Where:** `src/rules/derived.mts` `deriveCombatSlots` — `ownIndex = weapons.findIndex(w => w.qualities.includes("own"))`, then `buildSlot(weapons, ownIndex, …)`. The stored `character.combat.carried[2].weaponIndex` was never read (the hand slots did read `existing[0|1].weaponIndex`).
- **Impact:** A validated own-slot write was silently discarded. `validateCombatCarried` accepts any own-quality weapon for the own slot (ADR-014; `ES §carried-slots`), the PATCH returned 200, and recalc then overwrote the slot with the first own-quality weapon in `equipment.weapons[]`. Since `natural_weapon` is permanently index 0, War Claws / Battle Heels could never occupy the own slot. Blocked the Chunk I weapons picker (own-slot select would visibly snap back). Hand slots were unaffected.
...
- **Fix:** `deriveCombatSlots` resolves the own anchor in order: the stored `existing[2].weaponIndex` when it indexes an own-quality weapon → the first own-quality weapon (`findIndex`) → `natural_weapon` synthesis (unchanged). `ES §carried-slots` Engine bullet states the order. Tests: honored / not-own / out-of-range / missing stored index in `test/rules/combat.test.mts`; the repro as an API E2E in `test/api.test.mts` (test server now seeds `war_claws`). Fixed together with NB-50.
- **Status:** ✅ Resolved — Phase 6 Chunk I step ½ (2026-09-16). Filed 2026-09-02 during the Chunk I step-1 readiness review.

### NB-50. Per-field PATCH validators see the stored character, not the merged batch
- **Domain:** infra
- **Where:** `src/models/validation.mts` `validateCharacterUpdate` — the per-field loop called `validateFieldValue(field, value, character)` with the **stored** character as `allData`; the merged clone was only built afterwards for the cross-field / catalog pass.
- **Impact:** Any schema `validate:` hook that reads sibling fields evaluated against pre-PATCH state. Concrete case: `validateCombatCarried` checks `weaponIndex` range and the own-slot `own` quality against the stored `equipment.weapons[]`. A single atomic PATCH that removes a weapon and re-maps `combat.carried` in the same batch (the Chunk I picker contract) was validated as *new indices against the old array*. With the own slot at `natural_weapon` (index 0) this was coincidentally safe; once NB-49 let the own slot point at a later own-quality weapon, removing any lower-index weapon yielded a spurious 422 for a batch that is valid once merged.
- **Fix:** the merged clone is built **before** per-field validation — every permitted (non-FORBIDDEN) update is applied to `structuredClone(character)` and that clone is passed as `allData` to `validateFieldValue`; the same clone feeds the existing merged-state pass. An update whose apply throws (an earlier update in the batch replaced its parent with a primitive) is reported as a `VALIDATION` error rather than propagating. Tests in `test/validation.test.mts`: weapons shrink + carried re-map with a non-zero own index passes; the same carried tuple alone against the stored array fails; the primitive-parent batch is reported, not thrown.
- **Status:** ✅ Resolved — Phase 6 Chunk I step ½ (2026-09-16), same commit as NB-49. Filed 2026-09-02 during the Chunk I step-1 readiness review.
