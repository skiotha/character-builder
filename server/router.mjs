...
import { MIME_TYPES, CLIENT_DIR, ASSETS_DIR, DATA_DIR, API_ROUTE, LOCAL_ADDRESS } from "#config";
import { requireDmToken } from "./nagara/auth.mjs";
import * as nagara from "./nagara/index.mjs";
import * as backup from "./nagara/backup.mjs";
...
} from "./nagara/handlers/index.mjs";
import {
  renderInitialView,
  renderCreationView,
  renderDashboardView,
  renderCharacterView,
} from "./nagara/renderers/index.mjs";
import {
  createViewRoute,
  createCharacterRoute,
} from "./nagara/routes/index.mjs";
...
export default async function app(req, res) {
  ...
  if (pathname.startsWith("/assets/")) {
    return serveStaticFile(ASSETS_DIR, pathname.substring("/assets".length), res);
  }
...
async function serveClient(pathname, res) {
  try {
    ...
    const fullPath = path.join(CLIENT_DIR, normalizedPath);
